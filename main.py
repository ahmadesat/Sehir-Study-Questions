...
def searchfor():
	...
	if request.method == "GET":
		...
		return redirect("/{name_of_course}".format(name_of_course=searchquery))
	...
	if 'loggedin' in session:
		...
		if request.method == "GET":
			...
			if domain == "sehir.edu.tr":
				cursor.execute("select * from Courses WHERE course_name=%s and course_prof=%s",
...
def viewAllAnswers(name_of_course, question_id):
	if 'loggedin' in session:
		...
		if request.method == "GET":
			cursor.execute(
				"select  answer.description, student.name, status, answer.postingDate from Answer,Student WHERE answer.posted_by=student.id and Answer.q_id = %s",
				(question_id,))
